...
function squidGuardMiddleware(excludedPaths = [], allowedOrigins = []) {
  return (req, res, next) => {
    ...
    // If allowedOrigins is specified, only monitor requests from those origins
    if (allowedOrigins.length > 0) {
      const origin = req.get('origin') || '';
      const referer = req.get('referer') || '';
      
      // Normalize allowed origins (remove protocol, handle ports)
      const normalizedAllowed = allowedOrigins.map(domain => {
        return domain
          .replace(/^https?:\/\//, '')  // Remove http:// or https://
          .split(':')[0];  // Get just the hostname (ignore port for matching)
      });
      
      // Extract origin from request
      let requestOrigin = '';
      if (origin) {
        requestOrigin = origin.replace(/^https?:\/\//, '').split('/')[0].split(':')[0];
      } else if (referer) {
        try {
          const url = new URL(referer);
          requestOrigin = url.hostname;
        } catch (e) {
          // Invalid URL, skip
        }
      }
      ...
      const isAllowed = normalizedAllowed.some(allowed => {
        return requestOrigin === allowed || requestOrigin.includes(allowed) || allowed.includes(requestOrigin);
      });
      
      // Skip monitoring if not from allowed origin
      if (!isAllowed && requestOrigin) {
        return next();
      }
      
      // If no origin/referer, allow it (might be direct API call or same-origin)
